package com.yxw.weixin.mp.handler;
...
import com.yxw.weixin.mp.builder.TextBuilder;
...
public class MsgHandler extends AbstractHandler {
    // 用户发送手机验证码提示
    @Value("${yxw.weixin.registration.code.message}")
    private String registrationCodeMessage;
    // 默认用户发送验证码提示
    @Value("${yxw.weixin.default.registration.code.message}")
    ...
    public WxMpXmlOutMessage handle(WxMpXmlMessage wxMessage, Map<String, Object> context, WxMpService weixinService,
        ...
        if (RegexUtils.checkMobile(fromContent)) {
            ...
            if (!Constants.HTTP_RES_CODE_602.equals(exitMobile.getRtnCode())) {
                //其他错误,返回错误信息
                return new TextBuilder().build(exitMobile.getMsg(), wxMessage, weixinService);
            }
